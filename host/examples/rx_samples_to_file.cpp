...
bool check_locked_sensor(std::vector<std::string> sensor_names, const char* sensor_name, get_sensor_fn_t get_sensor_fn, double setup_time){
    ...
    while (true) {
        if ((not first_lock_time.is_not_a_date_time()) and
                (boost::get_system_time() > (first_lock_time + boost::posix_time::milliseconds(int(1e3 * setup_time)))))
        {
        ...
        if (get_sensor_fn(sensor_name).to_bool()){
            ...
            if (boost::get_system_time() > (start + boost::posix_time::milliseconds(int(1e3 * setup_time)))){
                std::cout << std::endl;
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    boost::this_thread::sleep(boost::posix_time::milliseconds(int(1e3 * setup_time))); //allow for some setup time
